...
# 5. 采集配置
TARGET_VALID_FRAMES = 20  # 目标有效帧数（收集到该数量才停止）
...
def collect_frames_and_calculate():
    ...
    aruco_detector = aruco.ArucoDetector(ORIGINAL_ARUCO_DICT, detector_params)
    ...
    while valid_frame_count < TARGET_VALID_FRAMES and collected_frame_count < MAX_TOTAL_FRAMES:
        ...
        if ids is not None and len(ids) >= 2:
            ...
            for i in range(2):  # 仅绘制前两个ArUco码
                ...
                # 绘制位姿轴
                rvec, tvec = calculate_aruco_pose(aruco_3d_points, tag_corners, CAMERA_MTX, CAMERA_DIST)
                if rvec is not None and tvec is not None:
                    cv2.drawFrameAxes(undistorted_frame, CAMERA_MTX, CAMERA_DIST, rvec, tvec, TAG_SIZE/2, 2)
